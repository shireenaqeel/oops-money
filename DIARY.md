...
**How:** New pure client `src/utils/gemini.ts` — `buildMoneyContext()` makes a compact snapshot (budget, month spend, remaining, income, net saved, top-3 categories) and `askGeminiCoach(key, history, msg, data)` POSTs to Gemini `gemini-2.0-flash:generateContent` with a tight system persona + the chat history; returns `{ok,text}` with friendly codes for invalid-key / rate-limit / network. `testGeminiKey()` is a one-line liveness check. New key stored in `om_gemini_key` (context `geminiKey` + `setGeminiKey`). `PaisaChat.tsx` upgraded: key present → async coach with a "Paisa soch rahi hai…" typing indicator + AI badge; key absent → instant offline answers (unchanged). New Settings card `AICoachSettings.tsx`: get-free-key link (Google AI Studio), paste + **check & save** (only saves if the key works), change/turn-off, and an **honest privacy note** — with the coach ON a short money summary goes to Google's API; offline chat sends nothing.
**Files changed:** `src/utils/gemini.ts` (new), `src/components/AICoachSettings.tsx` (new), `src/components/PaisaChat.tsx` (key-aware), `src/storage/index.ts` (+`om_gemini_key`), `src/hooks/useAppContext.tsx` (+`geminiKey`/`setGeminiKey`), `src/screens/SettingsScreen.tsx` (mounts the card).
**How to test on phone (needs internet):** Settings 🎀 → AI COACH 🤖 → tap **🔗 get a free key** → in Google AI Studio make a free key → paste it → **check & save** (turns green ✨). Now Insights → Paisa AI card → **💬 Paisa se pucho** shows an **AI ✨** badge → ask "is mahine kaise bachaun?", "am I overspending on food?" → real conversational replies using your numbers. Remove the key → chat goes back to offline. Turn off wifi with a key set → it falls back to offline answers with a note.
**Note:** free Gemini tier has rate limits (plenty for personal use); for a **publicly distributed** APK the key shouldn't be shipped in-app (use a free serverless proxy) — fine as-is for personal use.
**Next up:** (optional) an AI-written Monthly Wrapped narrative when a key is set; per-message "why" grounding.
...
- **Daily log sheet** (Clue-style) — per day: mark **period started / ended**, pick **flow** (spotting/light/medium/heavy), toggle **symptoms** (cramps, bloating, cravings, headache, etc.), pick a **mood**.
- **Period end tracking** → the app now knows your **real period length**, not a fixed 5 days.
- **Auto-learned predictions** — instead of trusting the manual stepper, it computes **your own average cycle length** (median of gaps between logged periods) and **average period length** from history. The manual stepper is now a labelled fallback.
- **"Your cycle" stats card** — avg cycle, avg period, regular vs irregular (with your shortest–longest range), and your **next 3 predicted period dates**.
**Why:** She asked to study real period apps (Flo, Clue, Stardust) and build the best of all. Those apps' core loop is: a month calendar you tap to log flow/symptoms/mood, real period-length learning, and predictions that adapt to *you* — none of which the earlier ring-only version had.
**How:** Fully **additive + backward-compatible** — the existing `periodStarts` data is untouched. Two new storage keys: `om_period_ends` (`{startISO: endISO}`) and `om_cycle_day_logs` (`{dateISO: {flow,symptoms,mood,note}}`). New context actions `setPeriodEnd` + `logCycleDay`. `cycle.ts` gained `getCycleStats` (median cycle gap + avg period + regularity), `effectiveCycleLength/effectivePeriodLength` (learned value wins over manual), `nextPeriods`, and `cycleMarksForRange` (powers the calendar; blends actual logged ranges with future projections). Ring + hero card now use the *effective* (learned) lengths.
**Files changed:** `src/types/index.ts` (+`FlowLevel`, `CycleDayLog`), `src/constants/cycleLog.ts` (new — flow/symptom/mood options), `src/storage/index.ts` (+2 keys), `src/hooks/useAppContext.tsx` (+state, +`setPeriodEnd`/`logCycleDay`), `src/utils/cycle.ts` (stats + calendar maths), `src/components/CycleCalendar.tsx` (new), `src/components/DayLogSheet.tsx` (new), `src/components/CycleRing.tsx` (+`periodLen`), `src/screens/CycleModal.tsx` (calendar + stats + day sheet).
**How to test on phone:** Settings 🎀 → **🌸 Cycle Tracking**. In the calendar, **tap today** → sheet opens → tap **🌸 period shuru hua**, pick a **flow**, toggle a couple of **symptoms** + a **mood** → **save**. The day turns rose with a 🩸 and a little dot. Tap a later day → **✅ period khatam hua** → the days in between fill in as period, and the **avg period** stat appears. Log two periods a few weeks apart → the **avg cycle** stat + **next 3 periods** show, and the ring countdown uses *your* number. Scroll around months with ‹ ›. Old logged periods still show in **LOGGED PERIODS** below.
**PCOS / irregular add-on (same day):** Since the target user (Indian women 18–28) has a high rate of PCOS, added four PCOS-focused, non-clinical extras:
- **Irregular / PCOS mode toggle** — when ON, ovulation/fertile predictions are hidden on the calendar *and* the prediction strip (calendar-method ovulation is unreliable for irregular cycles — this is the medically-responsible move), next-period shows as a "~estimate", and the "irregular" stat is reframed supportively ("common with PCOS, you're not alone"). Stored in new key `om_cycle_irregular`.
- **Auto-suggestion** — if cycles vary by >9 days and the mode is off, a gentle tap-to-enable chip appears.
- **PCOS-relevant symptoms** — added hair fall 💇‍♀️, extra hair 🧴, weight change ⚖️ to the daily-log chips.
- **🩺 Doctor summary + share** — a "for your doctor" card composes a plain-text summary (avg cycle, range, avg period length, rhythm, last 6 period dates, top 5 symptoms with counts) and shares it via the OS share sheet — handy for gynae/PCOS check-ups. Uses `topSymptoms()` (new in `cycle.ts`) + `Share.share`.
- **Longer cycles** — `getCycleStats` now accepts gaps up to 90 days (was 60) so long PCOS cycles still average correctly.
No diagnosis anywhere — the app only tracks and gently reflects; it never claims/labels a condition.
**Files also changed:** `src/constants/cycleLog.ts` (+3 symptoms), `src/utils/cycle.ts` (`topSymptoms`, wider gaps), `src/storage/index.ts` (+`om_cycle_irregular`), `src/hooks/useAppContext.tsx` (+`cycleIrregular`/`setCycleIrregular`), `src/components/CycleCalendar.tsx` (+`hideFertile`).
**How to test the PCOS bits:** Cycle space → turn on **irregular / PCOS cycles?** → the ovulation/fertile cells + calendar shading disappear, next period reads "~estimate". Log a few periods with uneven gaps → the range note becomes the supportive PCOS line, and (if off) the "cycles vary a lot" suggestion chip shows. Log some symptoms → **🩺 for your doctor → share summary** → your messaging app opens with a neat cycle summary.
**Next up:** (optional) symptom-pattern insight ("cramps usually start ~2 days before your period"), and a cycle-day mood/spend overlay in Insights.

## Feature — Cycle 🌸 its own space + better tracking — 23 Jun 2026
**What:** Period/cycle tracking now opens as its own full-screen space (a **Cycle Tracking** row in Settings 🎀, blush-pink), instead of being a 6th bottom tab. The page itself got a big redesign + smarter maths: a colour-coded **hero phase card** (Period / Fertile / PMS / Mid-cycle) showing your **cycle day** and a large **"next period in N days"** countdown; a 3-cell **prediction strip** for 🩸 next period, 🥚 ovulation, and 💕 fertile window; clear log buttons; a cycle-length card; logged-period history; and the **Cycle vs Money** insight — all in one sheet.
**Why:** Six bottom tabs was too crowded, so Cycle moved into a Settings entry (same pattern as Sapna Jar / Challenges). And real period apps (Flo, Clue) don't just track "period vs not" — they show fertile window + ovulation, so I added those predictions to make our tracking actually useful, not just a money gimmick.
**How:** Replaced the earlier `CycleScreen` (tab) + `CycleTracker` (component) with one self-contained `src/screens/CycleModal.tsx` (bottom-sheet, opened from Settings). Extended `src/utils/cycle.ts`: the `Phase` type gained `fertile`, and `getCycleInfo` now also returns `ovulation` (~14 days before next period), `fertileStart`, `fertileEnd`. Removed the Cycle tab + import from `App.tsx`. Deleted the two now-dead files. No storage change — same `om_periodStarts` / `om_cycleLength` keys, same spend maths.
**Files changed:** `src/screens/CycleModal.tsx` (new), `src/utils/cycle.ts` (fertile + ovulation), `App.tsx` (tab removed), `src/screens/SettingsScreen.tsx` (Cycle Tracking entry row + modal). Deleted: `src/screens/CycleScreen.tsx`, `src/components/CycleTracker.tsx`.
**How to test on phone:** Settings 🎀 → tap **🌸 Cycle Tracking** → ek bada sheet khulega. "period shuru hua aaj 🌸" ya "📅 koi aur din" se log karo → upar phase card mein cycle day + "agla period N din mein" dikhega, neeche 🩸/🥚/💕 prediction strip. Cycle length +/- karke prediction badalti dekho. Thodi spends ke baad **CYCLE vs MONEY** PMS vs baaki din compare karega. Confirm: bottom bar mein ab Cycle tab nahi hai (5 tabs).
**Flo-style ring (same day):** Replaced the flat hero card with a real **circular cycle ring** (`src/components/CycleRing.tsx`, drawn in react-native-svg). Each cycle day is one little arc coloured by its phase (🩸 period = rose, 💕 fertile = baby-blue, PMS = coral, normal = periwinkle), today is marked with a dot, and the centre shows a big **"N days to period"** countdown + cycle day. Added a small colour legend + the supportive phase line below it. New shared helper `phaseForCycleDay(day, cycleLength)` in `cycle.ts` keeps the ring and the maths in sync. Files: `src/components/CycleRing.tsx` (new), `src/utils/cycle.ts`, `src/screens/CycleModal.tsx`.
**Next up:** (optional) period-length logging (not just start dates), and a month calendar view.
